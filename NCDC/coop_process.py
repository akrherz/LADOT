...
o = open("563435547791dat.txt", 'r').readlines()
s = mx.DateTime.DateTime(2010,1,1)
e = mx.DateTime.DateTime(2011,8,1)
...
while (now < e):
  for sid in data.keys():
    ...
    sql = "INSERT into alldata_la (station, day, high, low, precip, snow, \
           sday, year, month,snowd) values ('%s','%s',%s,%s,%s,%s,'%s',%s,%s,%s) "\
           % (sid, now.strftime("%Y-%m-%d"), high, low, precip, snow,\
